            ...
            for root, dirs, files in os.walk(directory):
                for file in files:
                    ...
                    if file.startswith('._'):
                        self.logger.info(f"Removing macOS metadata file: {file}")
                        try:
                            file_path.unlink()
                        except Exception as e:
                            self.logger.warning(f"Could not delete metadata file {file}: {e}")
                        continue
    ...
    def cleanup_empty_dirs(self, directory):
        ...
        try:
            for root, dirs, files in os.walk(directory, topdown=False):
                # Filter out macOS metadata files when checking directory contents
